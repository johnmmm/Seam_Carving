...
void doubleCarving(cv::Mat& image,cv::Mat& outImage)//双向裁剪
{
    ...
    cMat[HNUM].copyTo(outImage);
}

void enlarge(cv::Mat& image,cv::Mat& outImage, cv::Mat& outMinTrace,cv::Mat& outLargeLine)
{
    cv::Mat image_gray(image.rows,image.cols,CV_8U,cv::Scalar(0));  //scalar 初始化为0 cv_8u 单通道阵列
    cv::cvtColor(image,image_gray,CV_BGR2GRAY); //彩色图像转换为灰度图像
...
void enlargeLines(cv::Mat& inImage,cv::Mat&inTrace,cv::Mat& outImage, cv::Mat& matrecord)
{
...
int main(int argc,char** argv)
{
    ...
    //doubleCarving(tmpMat, outImage);
    
    
    for(int i = 0; i < LNUM; i++)
    {
        enlarge(tmpMat, outImage, traces[i], largeLines[i]);
    ...
    for(int i = 0; i < LNUM; i++)
    {
        enlargeLines(tmpMat, traces[i], outImage, matrecord);
        tmpMat = outImage;
    }
...
//    }
    ...
    cv::imwrite("/Users/mac/Desktop/myself/pictures/my space/output_large.bmp", outImage);
